...
<body>
    ...
    <div class="tabs">
        ...
        <a href="stockmanager.html">Stock Manager</a>
        <a href="stomantask.php">Task</a>
        <a href="stomanattendance.php">Attendance</a>
        <a class="active" href="stomansalary.php">Salary</a>
    </div>

    <div class="container">
        <form action="stomansalaryfind.php" method="POST">
    ...
    $salary_query = "
        ...
        FROM salarytype
        WHERE salarytype IN ('Stock Manager Basic', 'Stock Manager Overtime', 'Stock Manager Bonus')
    ";
    ...
    if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
            if ($row['salarytype'] == 'Stock Manager Basic') {
                $basic = $row['salary'];
            } elseif ($row['salarytype'] == 'Stock Manager Overtime') {
                $overtime_rate = $row['salary'];
            } elseif ($row['salarytype'] == 'Stock Manager Bonus') {
                $bonus = $row['salary'];
            }
        }
    }
