...
<body>
    ...
    <div class="tabs">
        ...
        <a href="cashempcashier.html">Cashier</a>
        <a href="cashtask.php">Task</a>
        <a href="cashattendance.php">Attendance</a>
        <a class="active" href="cashsalary.php">Salary</a>
    </div>

    <div class="container">
        <form action="cashsalaryfind.php" method="POST">
    ...
    $salary_query = "
        ...
        FROM salarytype
        WHERE salarytype IN ('Cashier Basic', 'Cashier Overtime', 'Cashier Bonus')
    ";
    ...
    if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
            if ($row['salarytype'] == 'Cashier Basic') {
                $basic = $row['salary'];
            } elseif ($row['salarytype'] == 'Cashier Overtime') {
                $overtime_rate = $row['salary'];
            } elseif ($row['salarytype'] == 'Cashier Bonus') {
                $bonus = $row['salary'];
            }
        }
    }
